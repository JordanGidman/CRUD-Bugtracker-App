...
var _bugViewJs = require("./bugView.js");
var _bugViewJsDefault = parcelHelpers.interopDefault(_bugViewJs);
const controlAddBug = function() {};
const checkBtnClicked = function(e) {
    //check btn clicked
    if (e.target.classList.contains(`cta-signin`)) _modelJs.controlSignin();
    if (e.target.classList.contains(`cta-signup`)) _modelJs.controlSignup();
};
const init = function() {
...
},{"./model.js":"Py0LO","./bugView.js":"kJLxY","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"Py0LO":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
parcelHelpers.export(exports, "addBug", ()=>addBug);
const state = {
    activeUser: {},
    search: {},
    accounts: []
...
class Bug {
    constructor(bugName, bugPriority, bugDescription, bugTags, bugFinder, bugStatus){
...
const newBug = new Bug(`UI Rendering twice`, `High`, `The UI of the basket is rendering twice and impacting performance`, [
    `UI`,
    "CSS",
    `Rendering`,
    `Design`
...
const controlSignin = function() {
    //capture user data
    let userUsername = document.querySelector(`.input-username`).value;
    let userPassword = document.querySelector(`.input-password`).value;
    //Check accounts array for matching username
    //If yes then check password is an exact match
    activeUser = state.accounts.find((acc)=>acc.userName === userUsername && acc.password === userPassword);
    if (activeUser) //If yes pull userData from the account object in storage
    //set the activeUser to the found account
    //render any bugs that are in the bugsArr
    //else error could not find account or incorrect password.
    //clear input fields
...
const controlSignup = function() {
    ...
    if (state.accounts.find((acc)=>acc.userName === userUsername)) {
        alert(`An Account with this name already exists`);
        return;
    }
    //create a new user object
    const newUser = new User(userUsername, userPassword);
...
const addBug = function(bugDetails) {};
